function out = zoomFFT_realunits_memory(f,lambda,a,Npup,u,Nimg,Ein)
...
out = czt_memory(czt_memory(Ein, 2*Nimg+1, W, A).',2*Nimg+1, W, A).'.* exp(-2*pi*i * (xs(1)*ones(2*Nimg+1,1)*xis' + ys(1)*etas*ones(1,2*Nimg+1))/(lambda*f)) * dx*dy /(lambda*f);
